...
rowx = ['Day','Month','Year','Hour','Weekday']
    ...
    for ry in rowy:
        ...
        for row1 in cr:
            if i==0:
                ...
                header1=row1[row_y]
                header2=row1[row_x]
            elif (row1[14]=="IN" and row1[8]=="00" and row1[15]=="POS"):
                if (row1[row_y]!="" and row1[row_x]!=""):
                    a.append(row1[row_y])
                    if rx=='Month':
                        b.append((row1[row_x]+";"+row1[p['Year']]))
                    else:
                        b.append(row1[row_x])
        y=set(a)
        z=set(b)
        ...
        for row in cr:
            if i==0:
                i += 1
            elif (row[14]=="IN" and row[8]=="00" and row[15]=="POS"):
                a=float(row[7].replace(",",""))
                if (row[row_y]!="" and row[row_x]!=""):                        
                    y_axis[row[row_y]]+= 1
                    if rx=='Month':
                        table[(row[row_y],(row[row_x]+";"+row[p['Year']]))]+=1
                        x_axis[(row[row_x]+";"+row[p['Year']])] += 1
                    else:
                        table[(row[row_y],row[row_x])]+=1
                        x_axis[row[row_x]] += 1
        
        #Writing into csv   
        c = csv.writer(open("F:\\Clink\\Analytics\\Output\\Heatmaps\\{}_{}_POS.csv".format(header1,header2), "wb"))    
        x=[]   
        y=[]
